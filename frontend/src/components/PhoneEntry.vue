<template>
  <div class="login-card">
    ...
    <input v-model="phone" class="phone-input" type="tel" maxlength="11" inputmode="numeric" placeholder="11 位手机号" autocomplete="off" @keydown.enter="confirm" />
    ...
    <button class="primary-btn" :disabled="loading" @click="confirm">{{ loading ? '查询中…' : confirmText }}</button>
...
const props = defineProps({
  title: { type: String, default: '请输入提货手机号' },
  subtitle: { type: String, default: '钢铁现货仓库 · 提货导航' },
  confirmText: { type: String, default: '查询我的提货单' }
});
...
async function confirm() {
  ...
  loading.value = true;
  try {
    const data = await fetchOrders(phone.value);
    emit('success', phone.value, data);
  } catch (e) {
