/**
 * 3D 厂区导航场景（Three.js）。框架无关：park/roads 经构造参数传入。
 * 渲染厂区外观 + 库位标记 + 路线 + 通行规则(磅房/单行箭头/限制路段)，以及库内灵活布局视图。
 */
import * as THREE from 'three';
import { OrbitControls } from 'three/examples/jsm/controls/OrbitControls.js';
import { layoutOf, resolveCell, locationToWorld as locToWorld } from './geo.js';
...
export class Warehouse3D {
  ...
  _layoutOf(wh) { return layoutOf(wh, this.grid); }
  _locWorld(wh, code) { return locToWorld(wh, code, this.grid); }
  ...
  _buildPark() {
    ...
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(PARK.parkSize.width, PARK.parkSize.depth), new THREE.MeshStandardMaterial({ color: 0x1b2a3d }));
  ...
  _buildExteriorGrid(group, wh) {
    const layout = this._layoutOf(wh); const Z = layout.length;
    const x0 = wh.x - wh.width / 2, z0 = wh.z - wh.depth / 2; const zoneW = wh.width / Z;
    const mat = new THREE.LineBasicMaterial({ color: 0x35506e, transparent: true, opacity: 0.5 });
    const v = (x, z) => new THREE.Vector3(x, 0.2, z); const pts = [];
    ...
    layout.forEach((zcfg, zi) => {
      ...
      for (let r = 0; r < rowCount; r++) { const cols = zcfg.rows[r]; const cw = zoneW / cols; const zt = z0 + r * rowD, zb = z0 + (r + 1) * rowD; for (let c = 1; c < cols; c++) { const x = zx + c * cw; pts.push(v(x, zt), v(x, zb)); } }
    });
  ...
  setItems(items) {
    ...
    items.forEach((it) => {
      ...
      const pos = this._locWorld(wh, it.locationCode);
      ...
      this.markers.set(it.key, { group, head, pin, tag, baseColor: color, pos });
    });
  }
  ...
  setRoute(plan) {
    ...
    plan.order.forEach((o, i) => { const num = makeTextSprite(`${i + 1}`, { fontSize: 60, bg: 'rgba(235,87,87,0.95)', pad: 24 }); num.position.set(o.entrance.x, 46, o.entrance.z); num.scale.multiplyScalar(1.2); this.routeGroup.add(num); });
  ...
  enterInterior(warehouseId, highlights) {
    ...
    this.parkGroup.visible = false; if (this.routeGroup) this.routeGroup.visible = false;
    this.markers.forEach((m) => (m.group.visible = false));
    this.mode = 'interior'; this.currentInterior = warehouseId;
    const cam = this.interiorGroup.userData.camera; this._startTween(cam.position, cam.target);
  ...
  _buildInterior(wh, highlights) {
    ...
    const hiCells = (highlights || []).map((h) => { const r = resolveCell(wh, h.code, this.grid); return { zi: r.zi, row: r.row, col: r.col, code: h.code, label: h.label }; });
    ...
    layout.forEach((zcfg, zi) => {
      ...
      const zlabel = makeTextSprite(`${zcfg.zone} 区`, { fontSize: 58, bg: 'rgba(47,128,237,0.95)', pad: 22 }); zlabel.position.set(zx + zw / 2, 26, totalD / 2 - 4); zlabel.scale.multiplyScalar(1.05); grp.add(zlabel);
      for (let r = 1; r <= rowCount; r++) {
        const cols = zcfg.rows[r - 1];
        const rlab = makeTextSprite(`${zcfg.zone}-${String(r).padStart(2, '0')}（${cols}位）`, { fontSize: 24, bg: 'rgba(20,30,45,0.82)' }); rlab.position.set(zx - 5, 5, rowZc(r)); grp.add(rlab);
        for (let c = 1; c <= cols; c++) { const cx = zx + (c - 0.5) * cellW; const cz = rowZc(r); const hi = isHi(zi, r, c); this._buildStack(grp, wh.stackType, cx, cz, cellW, cellD, hi ? stackHiMat : stackMat, !!hi); }
      }
    });

    const hiPts = [];
    hiCells.forEach((h) => {
      const cx = zoneStartX[h.zi] + (h.col - 0.5) * cellW; const cz = rowZc(h.row); hiPts.push({ x: cx, z: cz });
      const beam = new THREE.Mesh(new THREE.CylinderGeometry(1.1, 1.1, 40, 14), new THREE.MeshStandardMaterial({ color: HIGHLIGHT, emissive: HIGHLIGHT, emissiveIntensity: 0.6, transparent: true, opacity: 0.85 })); beam.position.set(cx, 20, cz); grp.add(beam);
      const ring = new THREE.Mesh(new THREE.RingGeometry(cellW * 0.55, cellW * 0.75, 28), new THREE.MeshBasicMaterial({ color: HIGHLIGHT, side: THREE.DoubleSide, transparent: true, opacity: 0.8 })); ring.rotation.x = -Math.PI / 2; ring.position.set(cx, 0.3, cz); grp.add(ring);
      const tag = makeTextSprite('📍 ' + (h.label ? `${h.label} · ${h.code}` : h.code), { fontSize: 38, bg: 'rgba(235,87,87,0.96)', pad: 16 }); tag.position.set(cx, 46, cz); tag.scale.multiplyScalar(1.1); grp.add(tag);
    });

    const ent = new THREE.Mesh(new THREE.ConeGeometry(4, 10, 4), new THREE.MeshStandardMaterial({ color: 0x27ae60 })); ent.rotation.x = Math.PI; ent.position.set(0, 7, totalD / 2 + 6); grp.add(ent);
    ...
    const nameLabel = makeTextSprite(`${wh.name} · 库内（${layout.length} 区，目标库位 ${hiCells.length} 个）`, { fontSize: 44, bg: 'rgba(20,40,70,0.95)' }); nameLabel.position.set(0, 60, -totalD / 2 - 6); grp.add(nameLabel);
