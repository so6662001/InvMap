 ...
 * 厂区 / 库房 / 道路配置（可定制）。
 *
 * 库内布局（灵活）：warehouse.layout = [{zone, rows:[每排库位数]}]，未配置时回退统一网格 grid。
 *
 * 道路属性（用于路线规划的通行规则）：每条道路段可带
 *   - oneway: 0 双向(默认) | 1 仅正向(横向沿 +X / 纵向沿 +Z) | -1 仅反向
 *   - maxHeight: 限高(米，0/缺省=不限)
 *   - maxWeight: 限重(吨，车货总重，0/缺省=不限)
 *   - speed: 限速(km/h，用于"时间最短")，默认 20
 *   - congestion: 拥堵系数(>1 更慢)，默认 1
 *   - closed: 是否封闭
 *
 * 磅房 weighbridge：进门称皮、出门称重的必经点。
 * 车型预设 VEHICLE_PRESETS：含车高/车货总重，用于按限高/限重过滤道路。
 *
 * 坐标系：x 向右、z 向后，单位米。
 */

const STORAGE_KEY = 'invmap_park_config_v3';
...
export const DEFAULT_PARK = {
  ...
  gate: { id: 'GATE_IN', name: '大门', x: -140, z: 120 },
  exit: { id: 'GATE_OUT', name: '出门口', x: -100, z: 120 },
  weighbridge: { id: 'WB', name: '地磅 / 磅房', x: -120, z: 100 },
  warehouses: [
...
export const DEFAULT_ROADS = {
  horizontals: [
    { z: 80, x0: -150, x1: 150, speed: 25 },
    { z: -20, x0: -150, x1: 150, speed: 15, congestion: 1.6 },         // 拥堵+限速：时间最短会尽量避开
    { z: -100, x0: -150, x1: 150, speed: 20, oneway: 1 }               // 单行：仅可 +X 行驶
  ],
  verticals: [
    { x: -140, z0: -100, z1: 130, speed: 20 },
    { x: 140, z0: -100, z1: 80, speed: 20, maxHeight: 3.5, maxWeight: 45 } // 限高3.5m/限重45t：半挂车需绕行
  ]
};
...
export function setActiveConfig(cfg) { active = { PARK: cfg.PARK, ROADS: cfg.ROADS || clone(DEFAULT_ROADS) }; return active; }
export function saveConfig(cfg) { const data = { PARK: cfg.PARK, ROADS: cfg.ROADS || clone(DEFAULT_ROADS) }; localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); active = data; return active; }
export function resetConfig() { localStorage.removeItem(STORAGE_KEY); active = { PARK: clone(DEFAULT_PARK), ROADS: clone(DEFAULT_ROADS) }; return active; }
